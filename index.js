  ...
  if (_handlers.length > 0) {
    _handlers.map(function (key) {
      if (key.action) {
        ...
        if (globalHandler) {
          if (key.validate) {
            actionCreators[key.name] = (0, _reduxActions.createAction)(model.namespace + '/' + key.name, globalHandler, key.validate);
          } else {
            actionCreators[key.name] = (0, _reduxActions.createAction)(model.namespace + '/' + key.name, globalHandler);
          }
        }
      } else {
